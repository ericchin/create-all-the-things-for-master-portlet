		...
		try {
			if (Validator.isNotNull(numberOfArticles) &&
				...
				for (int i = 1; i <= Integer.parseInt(numberOfArticles); i++) {
					...
					List<DDMStructure> ddmStructureList =
						DDMStructureLocalServiceUtil.getDDMStructures(
							QueryUtil.ALL_POS, QueryUtil.ALL_POS);

					DDMStructure ddmStructure = null;

					for (DDMStructure currDDMStructure : ddmStructureList) {
						if (currDDMStructure.getStructureKey().equals(
								structureKey)) {
							ddmStructure = currDDMStructure;
						}
					}

					List<DDMTemplate> ddmTemplateList =
						DDMTemplateLocalServiceUtil.getDDMTemplates(
							QueryUtil.ALL_POS, QueryUtil.ALL_POS);

					DDMTemplate ddmTemplate = null;

					for (DDMTemplate currDDMTemplate : ddmTemplateList) {
						if (currDDMTemplate.getClassPK() ==
								ddmStructure.getStructureId()) {
							ddmTemplate = currDDMTemplate;
						}
					}
					...
					JournalArticleServiceUtil.addArticle(
						...
						ddmTemplate.getTemplateKey(), //ddmTemplateKey
						null, //layoutUuid
						1, //displayDateMonth
						1, //displayDateDay
